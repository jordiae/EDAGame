...
struct PLAYER_NAME : public Player {
  ...
  inline bool extra_knighted(Pos pos) {
    if (knighted(pos)) {
      return true;
    }
    for (int i = 0; i < 8; i++) {
...
/*
  ...
  inline bool check_targetable_escape(Pos pos,UnitType unitType, int health) {
   ...
   inline bool check_farmer(Pos pos,UnitType unitType, int health, bool escape, bool first_pos) {
    ...
    if (unitType == Farmer) {
      ...
      if (c.type == Wall)
        return false;
      if (first_pos and not escape and extra_knighted(pos))
        return false;
      if (c.haunted)
        return false;
  ...
  inline bool bfs(int id, bool escape) {
    ...
    while (not q.empty()) {
      ...
      q.pop();
      for(int i = 0; i < n; i++) {
        if (check_farmer(pos+mov[i],unitType,u.health,escape,first_pos)) {
  ...
  void print_already_moved(int id) {
    set<Pos>::iterator it;
    cout << "Sóc " << id << ".Posicions ja agafades: " << endl;
    for (it=already_moved.begin(); it!=already_moved.end(); ++it)
      cout << ' ' << *it;
    cout << endl;
  }
