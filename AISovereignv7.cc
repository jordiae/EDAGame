 ...
 */
#define PLAYER_NAME Sovereignv7
...
/*
  ...
  inline bool check_targetable_escape(Pos pos,UnitType unitType, int health) {
   ...
   inline bool check_farmer(Pos pos,UnitType unitType, int health, bool escape) {
    if (unitType == Farmer) {
      Cell c = cell(pos);
      if (c.type == Wall)
        return false;
      if (c.haunted)
        return false;
      if (not escape and extra_haunted(pos)) // !
        return false;
      if (c.id != -1)
        return false;
      if (not escape and knighted(pos))
        return false;
      return true;
    }
    else if (unitType == Knight){
  ...
  inline bool bfs(int id, bool escape) {
    ...
    while (not q.empty()) {
      ...
      q.pop();
      for(int i = 0; i < n; i++) {
        if (check_farmer(pos+mov[i],unitType,u.health,escape) and not (first_move and already_moved.find(pos+mov[i]) != already_moved.end() )) {
          if (check_targetable(pos+mov[i],unitType,u.health,escape)) {
            ...
            cout << "sóc " << id << "i em mouré cap a " << pos_aux << endl;
            if (already_moved.find(pos_aux) != already_moved.end())already_moved_c++;
        ...
        else if (first_move and already_moved.find(pos+mov[i]) != already_moved.end())
          cout << "sóc " << id << " i em volia moure cap a " << pos+mov[i] << endl;
      }
      if (first_move)
        first_move = false;
    }
  ...
  virtual void play () {
    cout << "size: " << already_moved.size() << endl;
    already_moved.clear();

    moveAllUnits();
    cout << "invalids: " << already_moved_c << endl;
  }
