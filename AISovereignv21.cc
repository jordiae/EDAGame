 ...
 */
#define PLAYER_NAME Sovereignv21
...
struct PLAYER_NAME : public Player {
  ...
  inline bool check(Pos pos,UnitType unitType, int health, bool escape, bool first_pos, bool alternative) {
    ...
    if (unitType == Farmer) {
      ...
      if ((self_haunted(pos) > 1) and round() > 20) { // (abans self_haunted > 0)
        return false;
      }
  ...
  inline bool check_targetable(Pos pos,UnitType unitType, int health, bool escape) {
    if (not escape and unitType == Farmer and already_targeted.find(pos)!= already_targeted.end())
      return false;
    if (unitType == Farmer) {
      if (escape)
        return true;
  ...
  inline bool bfs(int id, bool escape, bool alternative) {
    ...
    while (not q.empty()) {
      ...
      q.pop();
      for(int i = 0; i < n; i++) {
        if (check(pos+mov[i],unitType,u.health,escape,first_pos, alternative)) {
          if (check_targetable(pos+mov[i],unitType,u.health,escape)) {
            ...
            if (unitType == Farmer)
              already_targeted.insert(pos+mov[i]);
  ...
  inline void moveWitch(int id) {
    bool b = bfs(id,false,false);
    if (not b)
      command(id,None);
  }

  inline void moveAllFarmers() {
    f = farmers(0);
    for (int i = 0; i < f.size(); i++)
      moveFarmer(f[i]);
  }

  inline void moveAllKnights() {
    k = knights(0);
    for (int i = 0; i < k.size(); i++)
      moveKnight(k[i]);
  }

  inline void moveAllWitches() {
    w = witches(0);
    for (int i = 0; i < w.size(); i++)
      moveWitch(w[i]);
  }

  inline void moveAllUnits() {
    moveAllWitches();
    moveAllKnights();
    moveAllFarmers();
  }
