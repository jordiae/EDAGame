 ...
 */
#define PLAYER_NAME SovereignTO
...
*/
  ...
  inline bool bfs_farmer(int id) {
    ...
    while (not q.empty()) {
      ...
      q.pop();
      for(int i = 0; i < n; i++) {
        if (not(already_moved.find(pos+mov[i]) != already_moved.end() and first_move)and check_farmer(pos+mov[i],unitType,u.health)) {
        //if (check_farmer(pos+farmer_movements[i])) {
  ...
  inline bool bfs_escape(int id) {
    ...
    while (not q.empty()) {
      ...
      q.pop();
      for(int i = 0; i < n; i++) {
        if (not(already_moved.find(pos+mov[i]) != already_moved.end() and first_move) and check_farmer_escape(pos+mov[i],unitType,u.health)) {
        //if (check_farmer(pos+farmer_movements[i])) {
